...
export const FROM_EMAIL = process.env.RESEND_FROM_EMAIL ?? 'noreply@conduitcrm.com';
export const FROM_NAME = 'Conduit CRM';
...
export async function sendPaymentFailedEmail(params: {
  ...
  await resend.emails.send({
    from: FROM,
    to: params.to,
    subject: 'Action required: Payment failed for Conduit CRM',
    html: `
      <div style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        <h2 style="color: #0F1D2E;">Payment Failed</h2>
        <p>We were unable to process a payment of <strong>$${(params.amount / 100).toFixed(2)}</strong> for your Conduit CRM account (<strong>${params.orgName}</strong>).</p>
        ...
        <p style="margin-top: 24px; color: #6C6C70; font-size: 14px;">If you have questions, contact <a href="mailto:support@conduitcrm.com">support@conduitcrm.com</a>.</p>
      </div>
    `,
  });
}
...
export async function sendSubscriptionCanceledEmail(params: {
  ...
  await resend.emails.send({
    from: FROM,
    to: params.to,
    subject: 'Your Conduit CRM subscription has been canceled',
    html: `
      <div style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        <h2 style="color: #0F1D2E;">Subscription Canceled</h2>
        <p>Your Conduit CRM subscription for <strong>${params.orgName}</strong> has been canceled.</p>
        ...
        <p style="color: #6C6C70; font-size: 14px;">Questions? <a href="mailto:support@conduitcrm.com">support@conduitcrm.com</a></p>
      </div>
    `,
  });
}
