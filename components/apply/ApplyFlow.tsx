...
// Shared 1003 flow used by BOTH the borrower-facing apply page and the internal
// lead "Application" tab, so every 1003 looks and works the same. Token-driven
// against the `applications` system. `embedded` drops the full-screen chrome for
// in-dashboard use.
...
export function ApplyFlow({ token, embedded = false }: { token: string; embedded?: boolean }) {
  ...
  const body = (() => {
    ...
    if (notFound || !application) {
      return <p className="text-gray-500 text-center py-16">This application was not found or has already been submitted.</p>;
    }
    return (
      <>
  ...
  if (embedded) {
    return (
      <div className="bg-white rounded-card border border-border p-5">
        {saving && <p className="text-xs text-gray-400 text-right mb-2">Saving…</p>}
        {body}
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F9F9F9]">
      ...
      <div className="max-w-lg mx-auto px-4 py-6">{body}</div>
    </div>
  );
}
