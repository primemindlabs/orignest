  ...
  return (
    <div className={`bg-white rounded-xl border border-[#E8E4DE] px-5 py-4 ${muted ? 'opacity-60' : ''}`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        {post.status !== 'draft' && (
          <span className="ml-auto text-[10px] font-medium px-1.5 py-0.5 rounded bg-[#F4F2EF] text-[#6B7B8D]">{post.status}</span>
        )}
      </div>
      ...
      {post.hashtags && !editing && <p className="text-sm text-[#876830] mt-2">{post.hashtags}</p>}
