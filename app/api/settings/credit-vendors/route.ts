/**
 * Phase 147 — credit-vendor connections (admin only). Mirrors /api/settings/lenders.
...
const VENDORS = ['generic', 'factual_data', 'cbc', 'meridianlink', 'xactus', 'credco'];
...
export async function GET() {
  ...
  const { data } = await sb.from('credit_vendor_connections').select('id, vendor, api_url, auth_type, account_id, api_key_enc, is_active, last_pull_at, last_error, created_at').eq('org_id', orgId).order('vendor');
...
export async function POST(req: Request) {
  ...
  if (!userId) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  if (!orgId) return NextResponse.json({ error: 'No org' }, { status: 403 });
  if (!ADMIN.includes(role)) return NextResponse.json({ error: 'Only admins can connect a credit vendor.' }, { status: 403 });

  const b = (await req.json().catch(() => ({}))) as Record<string, any>;
  const vendor = String(b.vendor ?? 'generic');
  const authType = String(b.auth_type ?? 'basic');
  ...
  const sb = createAdminClient();
  const { data: existing } = await sb.from('credit_vendor_connections').select('id, api_key_enc, api_secret_enc').eq('org_id', orgId).eq('vendor', vendor).maybeSingle();
  ...
  const { error } = await sb.from('credit_vendor_connections').upsert(row, { onConflict: 'org_id,vendor' });
  if (error) { console.error('[credit-vendors] save failed', error); return NextResponse.json({ error: 'Could not save the connection.' }, { status: 500 }); }
...
export async function DELETE(req: Request) {
  ...
  const sb = createAdminClient();
  await sb.from('credit_vendor_connections').delete().eq('id', id).eq('org_id', orgId);
  return NextResponse.json({ ok: true });
}
