...
export async function POST() {
  try {
    ...
    for (const loId of loIds) {
      ...
      // Refi opportunities awaiting outreach (Sprint 2 — Refi Watch)
      const { count: refiCount } = await sb
      ...
      const contextLines = [
        ...
        `Refi opportunities (past clients who could save $50+/mo): ${refiCount ?? 0}`,
