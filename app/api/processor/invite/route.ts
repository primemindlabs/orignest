...
export async function POST(request: Request) {
  ...
  if (!processorClerkId) {
    return NextResponse.json(
      { error: 'No AshleyIQ account found for that email. The processor must sign up first.' },
      { status: 404 }
    );
  }
        ...
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
          <div style="margin-bottom: 32px;">
            <div style="display: inline-flex; align-items: center; gap: 8px; margin-bottom: 24px;">
              ...
              <span style="font-size: 17px; font-weight: 600; color: #0F1D2E;">AshleyIQ</span>
            </div>
            ...
            <p style="color: #6C6C70; margin: 0 0 8px;">
              <strong>${org.name}</strong> has invited you to work on their loan files through AshleyIQ.
            </p>
            <p style="color: #6C6C70; margin: 0 0 24px;">
              ...
              all from your unified AshleyIQ processor dashboard.
            </p>
          </div>
          <a href="${acceptUrl}"
