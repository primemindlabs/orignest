...
export async function POST(_req: Request, { params }: Ctx) {
  ...
  const { data: item } = await sb.from('training_items').select('id').eq('id', itemId).eq('org_id', orgId).maybeSingle();
  ...
  await sb.from('training_item_completions').upsert(
    { org_id: orgId, training_item_id: itemId, user_id: me },
    { onConflict: 'training_item_id,user_id', ignoreDuplicates: true },
  );
  await sb.from('training_item_views').insert({ org_id: orgId, training_item_id: itemId, user_id: me });
