...
import { Search, Copy, Check, Files, Archive, Sparkles, Plus, Megaphone } from 'lucide-react';
...
export function AdLibraryClient({ initial }: { initial: Creative[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return creatives.filter((c) => {
      if (type !== 'all' && c.ad_type !== type) return false;
      if (platform !== 'all' && c.platform !== platform) return false;
      if (needle && !`${c.headline} ${c.primary_text ?? ''} ${c.description ?? ''}`.toLowerCase().includes(needle)) return false;
      return true;
    });
  ...
  async function duplicate(c: Creative) {
    ...
    try {
      const res = await fetch('/api/ad-center', {
        ...
        body: JSON.stringify({
          ad_type: c.ad_type, platform: c.platform, headline: `${c.headline} (copy)`,
          primary_text: c.primary_text, description: c.description, cta_type: c.cta_type,
          nmls_number: c.nmls_number, apr_disclosure: c.apr_disclosure,
        }),
      });
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
          ...
          <p className="text-[15px] font-semibold text-black">{creatives.length === 0 ? 'No saved creatives yet' : 'No creatives match your filters'}</p>
          <p className="text-[13px] text-label-2 max-w-sm">Generate compliant ad copy in the builder and save it here to reuse, duplicate, and adapt across campaigns.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filtered.map((c) => (
            <div key={c.id} className="bg-white border border-black/[0.06] shadow-sm rounded-2xl p-4 flex flex-col">
