...
/**
 * Phase 147 — credit-vendor connections (admin). Connect a tri-merge credit vendor
 * for origination pulls. Mirrors LenderConnectionsCard; never shows the credential.
 */
import { useEffect, useState, useCallback } from 'react';
import { CreditCard, Plus, Trash2, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';
...
const VENDORS = [
  ...
  { v: 'factual_data', l: 'Factual Data — coming soon' },
  { v: 'cbc', l: 'CBC / MeridianLink — coming soon' },
  { v: 'meridianlink', l: 'MeridianLink — coming soon' },
  { v: 'xactus', l: 'Xactus — coming soon' },
  { v: 'credco', l: 'CoreLogic Credco — coming soon' },
];
const AUTH = [{ v: 'basic', l: 'Basic auth' }, { v: 'bearer', l: 'Bearer token' }, { v: 'api_key', l: 'API key header' }, { v: 'none', l: 'None' }];
const blank = { vendor: 'generic', auth_type: 'basic', api_url: '', account_id: '', api_key: '', api_secret: '' };

export function CreditVendorsCard({ canManage }: { canManage: boolean }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const j = await fetch('/api/settings/credit-vendors').then((r) => r.json()).catch(() => ({}));
  ...
  const save = async () => {
    ...
    const r = await fetch('/api/settings/credit-vendors', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
    setBusy(false);
  ...
  const remove = async (id: string, vendor: string) => {
    if (!confirm(`Disconnect ${vendor}? Past pulls are kept for audit.`)) return;
    await fetch(`/api/settings/credit-vendors?id=${id}`, { method: 'DELETE' }); load();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <div>
          <h2 className="text-[16px] font-bold text-[var(--c-text)] tracking-tight flex items-center gap-2"><CreditCard size={16} className="text-[var(--c-gold-deep)]" /> Credit Vendors</h2>
          <p className="text-[13px] text-[var(--c-label2)] mt-0.5">Connect a tri-merge credit vendor to pull scores during origination. Any vendor with a JSON pull endpoint works via “Generic”.</p>
        </div>
      ...
      {loading ? <div className="flex items-center gap-2 text-[13px] text-[var(--c-label2)] py-4"><Loader2 size={14} className="animate-spin" /> Loading…</div> : (
        <div className="space-y-2">
          {conns.length === 0 && !showForm && <div className="text-[13px] text-[var(--c-label2)] border border-dashed border-[var(--c-border)] rounded-[12px] px-4 py-6 text-center">No credit vendors connected yet.</div>}
          {conns.map((c) => (
          ...
          {showForm && canManage && (
            <div className="border border-[var(--c-border)] rounded-[12px] p-4 space-y-3 bg-[var(--c-fill)]">
              ...
              <Field label="Pull endpoint (JSON URL)"><input value={form.api_url} onChange={(e) => setForm({ ...form, api_url: e.target.value })} placeholder="https://vendor.example.com/api/credit/pull" className={inputCls} /></Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Account / subscriber #"><input value={form.account_id} onChange={(e) => setForm({ ...form, account_id: e.target.value })} className={inputCls} /></Field>
