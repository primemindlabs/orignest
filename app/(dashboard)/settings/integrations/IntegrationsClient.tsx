...
const LOS: Los[] = [
  ...
  { id: 'arive', name: 'Arive', desc: 'Sync loans & leads with Arive (OAuth). On connect we import your pipeline into Inbound and keep it current on a schedule. Copy all four values from Arive → Settings → API Integrations (Broker Pro / Non-Del plan). Base URL is your Arive web address, e.g. https://2393595.myarive.com.', fields: [{ key: 'client_id', label: 'ARIVE Client ID' }, { key: 'client_secret', label: 'ARIVE Secret Key' }, { key: 'api_key', label: 'ARIVE API Key' }, { key: 'base_url', label: 'ARIVE Base URL', type: 'text' }], webhook: { path: '/api/webhooks/arive', tenant: true, auth: 'shared' }, testable: true },
          ...
          <div key={los.id} className="bg-[var(--c-surface)] border border-[var(--c-border)] rounded-[14px] p-4">
            ...
            {isOpen && !c && canManage && los.fields.length > 0 && (
              <div className="mt-3 pt-3 border-t border-[var(--c-border)] space-y-3">
                ...
                <Button onClick={() => connect(los.id)} disabled={busy || !form.api_key || (los.id === 'arive' && (!form.base_url || !form.client_id || !form.client_secret))}>{busy ? 'Saving…' : 'Save & connect'}</Button>
