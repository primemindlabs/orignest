...
// Import review queue. Loans imported from a CSV — or pulled from Arive — land
// here for the LO to review and promote into the pipeline; nothing hits the real
// pipeline until the LO opts in.
...
export function InboundClient({ initial }: { initial: StagedRow[] }) {
  ...
  async function syncArive() {
    ...
    try {
      ...
      toast.success(d.staged > 0 ? `${d.staged} new record${d.staged === 1 ? '' : 's'} from Arive added to review` : 'Arive synced — no new records');
      router.refresh();
